...
class Config:
    ...
    RESONANCE_TOL = 0.01            # Tolerance for resonance tagging
...
class ChladniSimulator:
    ...
    def compute_lorentzian_weights(self, f_range: np.ndarray, target_freq: float) -> np.ndarray:
        """Compute Lorentzian weights for a specific mode across frequency range."""
    ...
    def get_mode_weight_at_frequency(self, f: float) -> np.ndarray:
        """Get Lorentzian weights for all modes at a given frequency."""
...
class ResonanceCurveWindow:
    ...
    def __init__(self, simulator: ChladniSimulator, main_ui):
        ...
        # New toggles
        self.amplitude_mode = False  # Default to original (intensity-like)
        self.normalize = False  # Default to absolute scale
        ...
        # Add checkboxes
        ax_amp_check = plt.axes([0.75, 0.01, 0.2, 0.04])
        self.amp_check = CheckButtons(ax_amp_check, ['Amplitude Mode'], [self.amplitude_mode])
        self.amp_check.on_clicked(self.toggle_amp_mode)

        ax_norm_check = plt.axes([0.75, 0.06, 0.2, 0.04])
        self.norm_check = CheckButtons(ax_norm_check, ['Normalize'], [self.normalize])
        self.norm_check.on_clicked(self.toggle_normalize)

    def toggle_amp_mode(self, label):
        self.amplitude_mode = not self.amplitude_mode
        self.setup_curve()

    def toggle_normalize(self, label):
        self.normalize = not self.normalize
        self.setup_curve()

    def compute_lorentzian(self, f_range: np.ndarray, target_freq: float, gamma: float) -> np.ndarray:
        """Compute Lorentzian (amplitude or intensity based on toggle)."""
        delta = f_range - target_freq
        if self.amplitude_mode:
            # Physical amplitude: peaks at 1/gamma
            lor = 1.0 / np.sqrt(delta**2 + gamma**2)
        else:
            # Original intensity-like: peaks at 1/gamma^2
            lor = 1.0 / (delta**2 + gamma**2)
        return lor

    def get_max_weight(self, gamma: float) -> float:
        """Get max Lorentzian value based on mode."""
        if self.amplitude_mode:
            return 1.0 / gamma
        else:
            return 1.0 / (gamma ** 2)

    def get_fwhm(self, gamma: float) -> float:
        """Compute FWHM based on mode."""
        if self.amplitude_mode:
            # For 1/sqrt(delta^2 + gamma^2), FWHM = 2 * gamma * sqrt(3)
            return 2 * gamma * np.sqrt(3)
        else:
            # For 1/(delta^2 + gamma^2), FWHM = 2 * gamma
            return 2 * gamma
    ...
    def setup_curve(self) -> None:
        ...
        # Compute max weight and FWHM
        gamma = self.simulator.gamma
        max_weight = self.get_max_weight(gamma)
        fwhm = self.get_fwhm(gamma)
        ...
        for (m, n), color in zip(self.current_modes, colors):
            lorentzian = self.compute_lorentzian(self.f_range, self.current_resonance_freq, gamma)
            if self.normalize:
                lorentzian /= max_weight
        ...
        self.ax.text(0.02, 0.98, f'γ = {gamma:.3f}',
                     transform=self.ax.transAxes, verticalalignment='top',
                     bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

        # Show FWHM (Full Width at Half Maximum)
        plot_max = 1.0 if self.normalize else max_weight
        half_max = plot_max / 2
        ...
        title = f'Lorentzian Resonance Curves for Mode(s): {modes_str}'
        if self.amplitude_mode:
        ...
        self.ax.set_ylabel('Resonance Weight' + (' (Normalized)' if self.normalize else ''))
        self.ax.legend()
        self.ax.grid(True, alpha=0.3)
        self.ax.set_xlim(f_min, f_max)
        self.ax.set_ylim(0, plot_max * 1.1)

        self.fig.canvas.draw_idle()

    def add_current_marker(self) -> None:
        """Add the current frequency marker to the plot."""
        # Compute current weight for the resonance mode
        gamma = self.simulator.gamma
        max_weight = self.get_max_weight(gamma)
        lorentzian_at_current = self.compute_lorentzian(np.array([self.current_f]), self.current_resonance_freq, gamma)[0]
        resonance_weight = lorentzian_at_current
        if self.normalize:
            resonance_weight /= max_weight
